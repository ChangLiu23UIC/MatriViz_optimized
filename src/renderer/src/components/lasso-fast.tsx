...
// Fast spatial index using Flatbush
class FastSpatialIndex {
  ...
  search(bbox: { minX: number; maxX: number; minY: number; maxY: number }): number[] {
...
const useLassoFast = ({
  ...
  const spatialIndexRef = useRef<FastSpatialIndex>(new FastSpatialIndex())
  ...
  const spatialIndex = useMemo(() => {
    ...
    const index = new FastSpatialIndex()
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent) => {
      ...
      if (selectionMode === 'rectangle' && selectionRect) {
        ...
        if (!lastPoint ||
            Math.abs(newPoint[0] - lastPoint[0]) > 5 ||
            Math.abs(newPoint[1] - lastPoint[1]) > 5) {
  ...
  const performSelection = useCallback(() => {
    ...
    if (selectionMode === 'rectangle' && selectionRect) {
      ...
      const indices = spatialIndex.search(bbox)
      selected = indices.map(index => spatialIndex.getPoint(index))
        .filter(point => isPointInRectangle(point, selectionRect))
      ...
      const indices = spatialIndex.search(bbox)
      selected = indices.map(index => spatialIndex.getPoint(index))
        .filter(point => isPointInCircle(point,
