...
// Ultra-lightweight spatial index - minimal memory footprint
class LightweightSpatialIndex {
  private gridSize = 50
...
// Ultra-fast point-in-polygon using integer math
const ultraFastPointInPolygon = (point: [number, number], polygon: [number, number][]): boolean => {
  const [x, y] = point
  let inside = false
...
const useLassoLightweight = ({
  ...
  const spatialIndexRef = useRef<LightweightSpatialIndex>(new LightweightSpatialIndex())
  const lastMouseMoveRef = useRef<number>(0)
  const mouseMoveThrottleRef = useRef<number>(32) // ~30fps - even more conservative
  ...
  const spatialIndex = useMemo(() => {
    ...
    const index = new LightweightSpatialIndex()
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent) => {
      ...
      // Very conservative throttling for maximum performance
      const now = Date.now()
      if (now - lastMouseMoveRef.current < mouseMoveThrottleRef.current) {
        return
      }
      ...
      if (selectionMode === 'rectangle' && selectionRect) {
        ...
        // Lasso mode - only update visualization, no selection computation
        ...
        if (!lastPoint ||
            Math.abs(newPoint[0] - lastPoint[0]) > 10 || // Higher threshold for fewer points
            Math.abs(newPoint[1] - lastPoint[1]) > 10) {
  ...
  const performSelection = useCallback(() => {
    ...
    if (selectionMode === 'rectangle' && selectionRect) {
      ...
      // Ultra-fast polygon check on candidates only
      selected = candidates.filter(point => {
        const x = point.cx || 0
        const y = point.cy || 0
        return ultraFastPointInPolygon([x, y], selectionPoints)
      })
    }
...
export default useLassoLightweight