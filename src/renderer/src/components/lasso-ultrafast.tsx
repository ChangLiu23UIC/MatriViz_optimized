...
// Ultra-fast spatial index using simple grid
...
const useLassoUltrafast = ({
  ...
  const spatialIndexRef = useRef<UltraFastSpatialIndex>(new UltraFastSpatialIndex())
  const lastMouseMoveRef = useRef<number>(0)
  const mouseMoveThrottleRef = useRef<number>(16) // ~60fps
  ...
  const spatialIndex = useMemo(() => {
    ...
    const index = new UltraFastSpatialIndex()
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent) => {
      ...
      if (selectionMode === 'rectangle' && selectionRect) {
        ...
        if (!lastPoint ||
            Math.abs(newPoint[0] - lastPoint[0]) > 8 ||
            Math.abs(newPoint[1] - lastPoint[1]) > 8) {
  ...
  }, [selectionMode, selectionRect, selectionCircle, lassoPoints, onSelection])
...
export default useLassoUltrafast